...
test('isola o lote por usuário e contabiliza resultados por cenário', () => {
  ...
  claimAutomationRun(internal, { name: 'Runner', version: '0.2.6', machine: 'QA-PC' });
...
test('recusa Runner antigo e mantém o lote disponível para reconexão após atualizar', () => {
  ...
  assert.equal(runnerVersionSupported('0.1.2'), false);
  assert.equal(runnerVersionSupported('0.2.0'), false);
  assert.equal(runnerVersionSupported('0.2.1'), false);
  assert.equal(runnerVersionSupported('0.2.2'), false);
  assert.equal(runnerVersionSupported('0.2.3'), false);
  assert.equal(runnerVersionSupported('0.2.4'), false);
  assert.equal(runnerVersionSupported('0.2.5'), false);
  assert.equal(runnerVersionSupported('0.2.6'), true);
  assert.throws(
    () => claimAutomationRun(internal, { name: 'Runner antigo', version: '0.1.1' }),
    (error) =>
      error.status === 426 &&
      error.code === 'AUTOMATION_RUNNER_UPDATE_REQUIRED' &&
      /versão 0\.2\.6 ou superior/.test(error.message)
  );
