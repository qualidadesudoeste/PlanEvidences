// Base de heurísticas de testes manuais. Cada categoria tem keywords que são
// matcheadas contra a HU pra decidir aplicabilidade. Categorias com sempreAplicavel
// aparecem em qualquer plano (filtradas só por aplicaApenasSe).

import type { QATipoSistema } from '@/types';
...
export interface SuiteCategoria {
  id: string;
  categoria: string;
  icone: string;
  keywords: string[];
  sempreAplicavel?: boolean;
  aplicaApenasSe?: (ctx: SuiteContext) => boolean;
  testes: string[];
}

export const SUITE_TESTES: SuiteCategoria[] = [
  {
    id: 'pre-requisitos',
  ...
  },
  {
    ...
    aplicaApenasSe: (ctx) =>
      ctx.tipoSistema === 'ia' ||
      /\b(ia|ai|chatbot|llm|gpt|assistente|machine learning|inteligência artificial)\b/i.test(ctx.hu),
    testes: [
